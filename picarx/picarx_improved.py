...
class Picarx(object):
    ...
    def __init__(self, 
        ...
        logging.info("Initializing Picarx")

        atexit.register(self.stop)
        logging.debug("Registered stop function with atexit")
    ...
    def set_motor_speed(self, motor, speed):
        ...
        type speed: int      
        '''
        logging.debug(f"Setting motor {motor} to speed {speed}")
        ...
        # if speed != 0:
        #     # speed = int(speed /2 ) + 50 #cause we don't want scaling
        #     pass
        # speed = speed - self.cali_speed_value[motor]

        logging.debug(f"Motor {motor + 1}: Direction set to {direction}, Speed set to {speed}")

        if direction < 0:
            self.motor_direction_pins[motor].high()
            self.motor_speed_pins[motor].pulse_width_percent(speed)
            logging.info(f"Motor {motor + 1} running backward at speed {speed}")
        else:
            self.motor_direction_pins[motor].low()
            self.motor_speed_pins[motor].pulse_width_percent(speed)
            logging.info(f"Motor {motor + 1} running forward at speed {speed}")

        logging.debug(f"Motor {motor + 1} set to speed {speed}")
    ...
    def backward(self, speed):
        logging.info(f"Moving backward with speed {speed}")
        current_angle = self.dir_current_angle
        logging.debug(f"steering angle: {current_angle}")
        ...
        if current_angle != 0:
            ...
            l = 95   # front and rear - mm
            d = 110  # wheels - mm
            angle_rad = math.radians(current_angle)

            # Ackermann principle --
            turning_radius = l / math.tan(angle_rad)
            v_r = speed * (turning_radius - (d / 2)) / turning_radius #speed of right wheel while turning
            v_l = speed * (turning_radius + (d / 2)) / turning_radius #left wheel

            v_l = constrain(v_l, -100, 100) #trying to avoid motor overload
            v_r = constrain(v_r, -100, 100)

            logging.info(f"Speed -- Left: {-v_l:.2f}, Right: {v_r:.2f}")

            self.set_motor_speed(1, -v_l) #left --backwards ; right --fwd for backwards movement
        ...
        logging.info("Backward executed.")


    def forward(self, speed):
        logging.info(f"Moving forward with speed {speed}")
        current_angle = self.dir_current_angle
        logging.debug(f"steering angle: {current_angle}")
        ...
        if current_angle != 0:
            ...
            turning_radius = l / math.tan(angle_rad)
            v_r = speed * (turning_radius - (d / 2)) / turning_radius
            v_l = speed * (turning_radius + (d / 2)) / turning_radius

            v_l = constrain(v_l, -100, 100)
            v_r = constrain(v_r, -100, 100)
    ...
    def stop(self):
        ...
        for _ in range(2):
            ...
            time.sleep(0.002)
        logging.debug("Motors stopped successfully")
